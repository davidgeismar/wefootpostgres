...
module.exports = {
	...
	// getTerrainsFree: function (req, res, next){
	...
	// 		var date = moment(req.param('date')).format();

	// 	Reservation.find({terrain: terrainId , date: date}, function foundReservation (err, reservation) {
	// 		if(err) return res.status(400).end(); 
	...
	// 				if(promotion){
	// 					var numterrain =promotion.terrain;
	// 					Terrainsfree[0].prix = promotion.promo*Terrainsfree[0].prix;
	// 				}else{
	// 					Terrainsfree[0].prix = Terrainsfree[0].prix;
	// 					var numterrain =Terrainsfree[0].id;
	// 				}
			
	// 				var rep = [numterrain,Terrainsfree[0]];
	...
	create: function (req,res,next){
		Reservation.create( {id:req.param('id'),date:req.param('date'),terrain:req.param('numTerrain')} , function reservationCreated(err,reservation){
			if(err){
				console.log(err);
				return res.status(400).end();
			}
